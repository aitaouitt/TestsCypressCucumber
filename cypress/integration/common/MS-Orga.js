...
let idOrg
  ...
  Given ('Creation d Organisation {string},{string},{string},{string},{string},{string},{string},{string}',
  (parent_id,adresse,CP,ville,precarity,SampleSocial,HighSocial,default_os) =>{
        ...
        cy.request({
          method: 'POST', 
          url: urlMS+"/organizations/"+vers+"/temp", 
          body: {
              "title": "Mon titre",
                "sigle": "MPZ",
        ...
        idOrg = response.body.$oid
        expect(response.status).equal(201)
      })
  })
  Given ('Ajout de l Organisation dans la table Def {string},{string},{string},{string},{string},{string},{string},{string}',
  (parent_id,adresse,CP,ville,precarity,SampleSocial,HighSocial,default_os) =>{
        let hight = parseInt(HighSocial)
        let sample = parseInt(SampleSocial)
        cy.request({
  ...
  And ('Update Organisation {string},{string},{string},{string},{string},{string},{string},{string}',
  (parent_id,adresse,CP,ville,precarity,SampleSocial,HighSocial,default_os) =>{
        cy.request({
          method: 'PATCH', 
  ...
  And ('Recherche Organisation {string},{string},{string},{string},{string},{string},{string},{string}',
  (parent_id,adresse,CP,ville,precarity,SampleSocial,HighSocial,default_os) =>{
        let hight = parseInt(HighSocial)
        let sample = parseInt(SampleSocial)
        cy.request({
  ...
  Given('Suppression Organisation',() => {
    cy.request({
      method: 'DELETE', 
      url: urlMS+"/organizations/"+vers+"/"+idOrg,
      body: {},
